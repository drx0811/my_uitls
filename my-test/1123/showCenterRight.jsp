...
<style>
    ...
    .label_img{ float: right; box-sizing: border-box; height: 24px; line-height: 24px; flex: 1; margin: 3px 0px 0px 0px; color: white; display: flex; justify-content: center; font-size: 16px; border-radius: 10px; }
                    ...
                    <div class="line">
                        <div class="video_card" style="flex: 8;margin-right: 1%;background: rgba(66,133,244,0.3);">
                            ...
                            <div class="good_view">
                                ...
                                <div class="good_mask" style="bottom: 12px;top: auto">
                                    <div :class=" (val.data && val.data.packageNo==currentRoll.packageNo)  ? 'roll_img current_roll':'roll_img' "
            ...
            <div class="body" :class=" firstVideo ? '':'opaticy_0' ">
                ...
                <div class="left">
                    ...
                    <div class="card_label">
                        ...
                        <div style="background: #3974e1;" class="label_img label_img2">
                            <div>出库中</div>
                            <div style="margin-right: 8px">{{totalOutNum}} 件</div>
                        </div>
                        <div style="background: #3e7430;" class="label_img label_img4">
                            <div>加工中</div>
                            <div style="margin-right: 8px">{{totalProNum}}件</div>
                        </div>
                         <div style="background: #ed6255;" class="label_img label_img3">
                            <div>期货</div>
                            <div style="margin-right: 8px">{{totalTimeNum}}件</div>
                        </div>
                        <div style="background: #fec469;" class="label_img label_img1">
                            <div>锁货中</div>
                        ...
                        <div id="carBoxId" class="line_new">
                            ...
                            <div v-if="currentWare=='4'" :class="['rows_new',((val.row ==1||val.row ==2) && currentWare=='4')?'rows_new_high':'']" v-for="val in showLeft">
                                ...
                                <img :id="roll.location" @click="onClickRoll($event,roll)"
                                     :class="['image_roll_new',currentWare=='4'?'image_roll_out_new':'',getRollClass(roll),(roll.data && selRoll.packageNo==roll.data.packageNo) ? 'roll_active_new' : '']"
                                     v-for="roll in val.list"
                                     :style=" 'bottom:' + roll.y + '%;left:' + roll.x + '%;' + 'z-index:' + (roll.lay ==1?1000:2000) "/>
                            </div>
                            <div v-if="currentWare!='4'" class="rows_new rows_new_3" v-for="val in showLeft">
                                <img :id="roll.location" @click="onClickRoll($event,roll)"
                                     :class="['image_roll_new','image_roll_out_new_3',getRollClass(roll),(roll.data && selRoll.packageNo==roll.data.packageNo)  ? 'roll_active_new' : '']"
                                     v-for="roll in val.list"
      ...
      computed:{
        totalOutNum(){
          ...
          for(let i=0;i<iventList.length;i++){
            ...
            for(let j=0;j<list.length;j++){
              if(list[j] && list[j].data && list[j].data.type=='02'){
                num = num + 1
              }
            }
          }
        ...
        totalProNum(){
          ...
          for(let i=0;i<iventList.length;i++){
            let list = iventList[i].list || []
            for(let j=0;j<list.length;j++){
              if(list[j] && list[j].data && list[j].data.type=='04'){
                num = num + 1
              }
            }
          }
        ...
        totalIssueNum(){
          ...
          for(let i=0;i<iventList.length;i++){
            let list = iventList[i].list || []
            for(let j=0;j<list.length;j++){
              if(list[j] && list[j].data && list[j].data.type=='03'){
                num = num + 1
              }
            }
          }
        ...
        totalTimeNum(){
          ...
          for(let i=0;i<iventList.length;i++){
            let list = iventList[i].list || []
            for(let j=0;j<list.length;j++){
              if(list[j] && list[j].data && list[j].data.type=='01'){
                num = num + 1
              }
            }
          }
    ...
    class VideoPlay {
        ...
        this.oWebControl = new WebControl({
          ...
          cbConnectError: () => { // 创建WebControl实例失败
            ...
            $("#"+this.id).html("插件未启动，正在尝试启动，请稍候...");
              this.oWebControl&&this.oWebControl.JS_WakeUp("VideoWebPlugin://"); // 程序未启动时执行error函数，采用wakeup来启动程序
            this.initCount ++;
            if (this.initCount < 3) {
              setTimeout( () => {
                this.initPlugin();
      ...
      init(){
        this.getPubKey(() => {
          ...
          ////////////////////////////////// 请自行修改以上变量值	////////////////////////////////////
            ...
            this.oWebControl&&this.oWebControl.JS_Resize( this.domWidth , this.domHeight );  // 初始化后resize一次，规避firefox下首次显示窗口后插件窗口未与DIV窗口重合问题
            this.isClean();
            this.startPreview();
          });
        });
      ...
      setWndCover() {
        ...
        iCoverBottom = (iCoverBottom > height) ? height : iCoverBottom;
          ...
          this.oWebControl&&this.oWebControl.JS_RepairPartWindow(0, 0, width+1, height);    // 多1个像素点防止还原后边界缺失一个像素条
        if (iCoverLeft != 0) {
            this.oWebControl&&this.oWebControl.JS_CuttingPartWindow(0, 0, iCoverLeft, height);
        }
        if (iCoverTop != 0) {
            this.oWebControl&&this.oWebControl.JS_CuttingPartWindow(0, 0, width+1, iCoverTop+1);    // 多剪掉一个像素条，防止出现剪掉一部分窗口后出现一个像素条
        }
        if (iCoverRight != 0) {
            this.oWebControl&&this.oWebControl.JS_CuttingPartWindow(width - iCoverRight, 0, iCoverRight, height);
        }
        if (iCoverBottom != 0) {
            this.oWebControl&&this.oWebControl.JS_CuttingPartWindow(0, height - iCoverBottom, width, iCoverBottom);
        }
      }
    }
</script>
