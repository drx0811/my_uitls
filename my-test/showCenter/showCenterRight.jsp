...
<style>
  ...
  .container_mask{ width: 100%; height: 100%; position: absolute; top: 0; left: 0; background: transparent; z-index: 11; cursor: pointer; }
  ...
  .card_label{ width: 100%; height: 30px; position: absolute; top: 0; right: 0; box-sizing: border-box; }
  .lable_status{ height: 20px; margin: 5px 2px; float: right; }
  .label_img{ float: right; box-sizing: border-box; padding-left: 26px; height: 24px; line-height: 24px; flex: 1; margin: 3px 0px 0px 0px; color: white; display: flex; justify-content: center; font-size: 16px; }
  ...
  .label_img4{ background: center / 100% 100% no-repeat url("${ctxStatic}/images/label02.png"); }
  ...
  .rawCar .gou{ width: 30px; height: 24px; position: absolute;top: 0; left: -3.5px; background: 0 0 / 100% 100% no-repeat url("${ctxStatic}/images/goz3.png"); }
  .zgz{ width: 20px; height: 20px; position: absolute; top: 200px; left: -2.5px; background: center / 100% 100% url("${ctxStatic}/images/goz.png") }
  .image_roll{ height: 32%; width: 1.3%; content: url("${ctxStatic}/images/steel01.png"); position: absolute; bottom: 0; left: 0; }
  .image_roll1{ height: 32%; width: 1.3%; content: url("${ctxStatic}/images/steel05.png"); position: absolute; bottom: 0; left: 0; z-index: 99; }
  .image_roll2{ height: 32%; width: 1.3%; content: url("${ctxStatic}/images/steel04.png"); position: absolute; bottom: 0; left: 0; z-index: 99; }
  .image_roll3{ height: 32%; width: 1.3%; content: url("${ctxStatic}/images/steel02.png"); position: absolute; bottom: 0; left: 0; z-index: 99; }
...
<div id="app" v-cloak>
  ...
  <div class="container">
    <%--        TODO --%>
    <div class="container_mask"  @click="onClickMask($event)"></div>
    <div class="title">
      ...
      <div class="body" :class=" firstVideo ? '':'opaticy_0' ">
        ...
        <div class="left">
          ...
          <div class="card_label">
            <%--					<img src="${ctxStatic}/images/zherd.png" class="lable_status" >--%>

            <div class="label_img label_img2">
              <div>出库中</div>
              <div style="margin-right: 8px">{{totalOutNum}} 件</div>
            </div>
            <div class="label_img label_img4">
              <div>加工中</div>
              <div style="margin-right: 8px">{{totalProNum}}件</div>
            </div>
            <div class="label_img label_img3">
              <div>锁货中</div>
              <div style="margin-right: 8px">{{totalIssueNum}}件</div>
            </div>
            <div class="label_img label_img1">
              <div>期货中</div>
              <div style="margin-right: 8px">{{totalTimeNum}}件</div>
            </div>
            <img src="${ctxStatic}/images/steel01.png" class="lable_status" >
            <img src="${ctxStatic}/images/steel03.png" class="lable_status" >
            <img src="${ctxStatic}/images/steel02.png" class="lable_status" >
            <img src="${ctxStatic}/images/steel05.png" class="lable_status" >
            <img src="${ctxStatic}/images/steel04.png" class="lable_status" >
          </div>
          <div class="up">
  ...
  let appVue = new Vue({
    ...
    methods:{
      ...
      getLastSendDetail(){
        ...
        $.post("${ctx}/showCenter/getCarMonitor", {}, function(res){
          self.carMonitor = Object.freeze(res||[])
        });
        $.post("${ctx}/showCenter/getLastSendDetail", {}, function(res){
          self.lastSendDetail = Object.freeze(res||[])
      ...
      getRowRoll(roll){
        ...
        $.post("${ctx}/showCenter/showInventoryByQu", { qu:'4' }, function(res){
          ...
          self.rollList = Object.freeze(list)
        });
      },
      onClickMask(e){
        console.log(e,'drx---1')
      ...
      getTopData(){
        ...
        $.post("${ctx}/showCenter/getTopDataInfoDay", {}, function(res){
          ...
          $('#processNum').text(parseInt(res['累计加工量']) || '0')
        });
      },
      getBarrierInfo(){
        let self = this;
        $.post("${ctx}/showCenter/getEnterOutBarrier", { operType: '3' }, function(res){
          let list = res || []
          for(let i=0;i<list.length;i++){
            let time = new Date();
            if(list[i].inDate){
              time = new Date(list[i].inDate)
            }
            list[i].dateText = (time.getMonth()+1).toString().padStart(2,'0') +'-'+
                time.getDate().toString().padStart(2,'0')+' '+
                time.getHours().toString().padStart(2,'0') +':'+
                time.getMinutes().toString().padStart(2,'0');
            list[i].carPlateNo = self.fillEnd(list[i].carPlateNo,8)
            list[i].statusDesc = list[i].status=='1' ? '人工开闸' : '自动开闸';
          }
          if(list.length<5){
          ...
          self.enterBarrier = Object.freeze(list)
        });
        $.post("${ctx}/showCenter/getEnterOutBarrier", { operType: '4' }, function(res){
          let list = res || []
          for(let i=0;i<list.length;i++){
            let time = new Date();
            if(list[i].inDate){
          ...
          self.outBarrier = Object.freeze(list)
        });
      },
      getBridgeInfo(){
        let self = this;
        $.post("${ctx}/showCenter/getEnterOutWeighbridge", { operType: '1' }, function(res){
          let list = res || []
          for(let i=0;i<list.length;i++){
            let time = new Date();
            if(list[i].endTime){
              time = new Date(list[i].endTime)
            }
            list[i].dateText = (time.getMonth()+1).toString().padStart(2,'0') +'-'+
                time.getDate().toString().padStart(2,'0')+' '+
                time.getHours().toString().padStart(2,'0') +':'+
                time.getMinutes().toString().padStart(2,'0');
            list[i].carPlateNo = self.fillEnd(list[i].carPlateNo,9)
            list[i].weightDesc = (list[i].weight||0) + '吨'
            list[i].weightDesc = self.fillEnd(list[i].weightDesc,7)
            list[i].statusDesc = list[i].status=='1' ? '正常' : '异常';
          }
          if(list.length<5){
            for(let i=list.length;i<5;i++){
              list.push({})
          ...
          self.enterBridge = Object.freeze(list)
        });
        $.post("${ctx}/showCenter/getEnterOutWeighbridge", { operType: '2' }, function(res){
          let list = res || []
          for(let i=0;i<list.length;i++){
            let time = new Date();
            if(list[i].endTime){
            ...
            list[i].dateText = (time.getMonth()+1).toString().padStart(2,'0') +'-'+
                ...
                time.getMinutes().toString().padStart(2,'0');
            list[i].carPlateNo = self.fillEnd(list[i].carPlateNo,9)
            list[i].weightDesc = (list[i].netWeight||0) + '吨'
            list[i].weightDesc = self.fillEnd(list[i].weightDesc,7)
            list[i].statusDesc = list[i].status=='1' ? '正常' : '异常';
          }
          if(list.length<5){
          ...
          self.outBridge = Object.freeze(list)
        });
      },
      ...
      togglePageVideo(key){
        if(key){
          ...
          areaPlay1 = new VideoPlay('down_video_box1','6ab1cf6d8d084f88ac1dcde5b7ca144f')
          if(areaPlay2){
          ...
          areaPlay3 = new VideoPlay('down_video_box3','934900954c4e498baea6a7691e7d1a09')
          if(areaPlay4){
            areaPlay4.removePlugin()
          }
          areaPlay4 = new VideoPlay('line_video1','bba61d1e1462435a85f6e79c973bfd32')
          if(areaPlay5){
          ...
          areaPlay6 = new VideoPlay('line_video3','a7a0fbc9ef9847ee86fa4de7cf3ae75b')
          if(areaPlay7){
    ...
    mounted(){
      ...
      if(this.currentWare!='4'){
        ...
        // this.getInventory('4',['A']);
      }
      setInterval(()=>{
        console.log("________________________________________________change");
        this.getTopData();
        this.getBarrierInfo();
        this.getBridgeInfo();
        this.getLastSendDetail();
        if(this.currentWare!='4'){
          ...
          let _qu = this.currentTrack;
          this.getInventory(_qu,[_qu]);
        }
        webSocketClient();
      },1000*60*300000)
      //TODO
      ...
      socket.onmessage = function(msg) {
        ...
        if(obj.deviceID != null && obj.deviceID !== ''){//稳定
          if(obj.deviceID==='01004'){
            ...
            $("#outBillCode").text(obj.taskCode||'');     //任务单号
          ...
          if(obj.deviceID==='01003'){
            ...
            $("#outBillCode2").text(obj.taskCode||'');     //任务单号
          ...
          if(obj.deviceID==='01001'){
            ...
            $("#inBillCode").text(obj.taskCode||'');     //任务单号
        ...
        cbConnectSuccess:  () => {                     // 创建WebControl实例成功
          ...
          this.oWebControl.JS_StartService("window", {         // WebControl实例创建成功后需要启动服务
            ...
            this.oWebControl.JS_SetWindowControlCallback({   // 设置消息回调
              cbIntegrationCallBack: (e)=>{
              }
            });
            this.oWebControl.JS_CreateWnd(this.id, this.domWidth , this.domHeight ).then( (e)=> { //JS_CreateWnd创建视频播放窗口，宽高可设定
